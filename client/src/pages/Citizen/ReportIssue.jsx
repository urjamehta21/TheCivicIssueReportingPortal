...
const ReportIssue = () => {
  ...
  const handleImageUpload = e => {
    ...
    if (selectedFiles.length === 0 && images.length >= 5) {
      toast.error('You can upload up to 5 images per report');
      return;
    }
    ...
    if (files.length > remainingSlots && remainingSlots > 0) {
      toast.info(`Only ${remainingSlots} more image slot${remainingSlots === 1 ? '' : 's'} available.`);
    }
